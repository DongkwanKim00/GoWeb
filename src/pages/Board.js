...
const Board = () => {
  ...
  return (
    <div style={{
      ...
      <h1 style={{ textAlign: "center", marginBottom: "50px", color: "black" }}>게시판</h1>
      <form onSubmit={handleSubmit}>
        <p style={{ color: "black" }}> 제목: <input type="text" className="form-control" placeholder="Title"
          aria-label="Title" required={true} value={title} onChange={handleChange_title}></input></p>
        <p style={{ color: "black" }}> 작성자: {author}</p>
        <p style={{ color: "black" }}> 카테고리:
        ...
        <textarea id="content" className="form-control" placeholder="Write your content here"
          ...
          style={{ height: "200px" }} />
        {/* 이미지 업로드 */}
        <p style={{ color: "black" }}>이미지 첨부: <input type="file" onChange={handleImageUpload} accept="image/*" /></p>
